                ...
                <div class="box-body">
                    <div class="tab-content p-3 mb-3">
                        ...
                        <div class="tab-pane animate fadeIn text-muted" id="tab2">
                            ...
                            <div class="row">
                                <div class="table-responsive">
                                    <table id="company" class="table table-bordered table-striped" data-plugin="dataTable">
                                      <thead class="text-white" style="background-color: #1089ff;">
                                          <tr>
                                              <th width="60px">No</th>
                                      ...
                                      <tbody>
                                          @foreach($comp as $key => $data)
                                            ...
                                            <tr>
                                                <td>{{$key+1}}</td>
                                                <td>{{getProfileCompany($data->id_profil)}}</td>
                                                <td>{{$data->email}}</td>
                                                <td><input type="checkbox" {{$check}} data-toggle="toggle" data-on="Active" data-off="Non Active" data-onstyle="info" data-offstyle="default" value="{{$data->id}}|{{$value}}" id="toggle_company_{{$data->id}}"></td>
                                            </tr>
                                          @endforeach
                                      </tbody>
                                    </table>
...
<script type="text/javascript">
	$(document).ready(function () {
        ...
        $('#company').DataTable({"columnDefs": [{ "orderable": false, "targets": 3 }]});
        $('#table').DataTable({
            processing: true,
            serverSide: true,
        ...
        $('input[type="checkbox"]').on('change', function(event){
            ...
            if (confirm('Are you sure?')) {
                ...
                $.ajax({
                  ...
                  data: {  "_token": "{{ csrf_token() }}", id:ambilsemua }
                }).done(function( msg ) {
                    if(msg == 'Success'){
                        if(ambil[1] == 1){
                            var hasil = ambil[0]+'|2';
