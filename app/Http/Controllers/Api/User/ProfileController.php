...
//            $id_profil = DB::table('itdp_company_users')->where('id', $id_user)->first()->id_profil;
            $path = ($idFoto) ? url('uploads/Profile/Eksportir/' . $id_user . '/' . $idFoto) : url('image/nia2.png');
...
//            $path = ($idaFoto) ? url('image/fotoprofil/' . $idaFoto) : url('image/nia2.png');
            $path = ($idaFoto) ? url('uploads/Profile/Importir/' . $id_user . '/' . $idaFoto) : url('image/nia2.png');
//            $destination= 'uploads\Profile\Importir\\'.$id_user;
//            $path2 = (string)Image::make($path)->resize(96, 96)->encode('data-url');
