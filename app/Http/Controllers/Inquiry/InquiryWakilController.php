...
class InquiryWakilController extends Controller
{
    ...
    public function create()
    {
        if(Auth::user()){
            ...
            if(Auth::user()->id_group == 4){
                ...
                return view('inquiry.perwakilan.form', compact('pageTitle', 'mode', 'url', 'id_country'));
            }else{
                return redirect('/home');    
            }
        }else{
            ...
            if(Auth::user()->id_group == 4){
                ...
                for ($k=0; $k <count($array) ; $k++) { 
                    ...
                    $notif = DB::table('notif')->insert([
                        ...
                        'url_terkait' => 'inquiry_perwakilan',
                        'status_baca' => 0,
                        'waktu' => $datenow,
                        'to_role' => 2,
                    ]);
            ...
            return \Yajra\DataTables\DataTables::of($user)
                ...
                ->addColumn('action', function ($mjl) use($id_user) {
                    if($mjl->status == 0){
                        ...
                        return '
                            <center>
                            <a href="'.url('/inquiry_perwakilan/chatting').'/'.$mjl->id.'" class="btn btn-sm btn-warning" style="color: white;"><i class="fa fa-comments-o" aria-hidden="true"></i> '.Lang::get('button-name.chat').' <span class="badge badge-danger">'.$this->getCountChat($mjl->id_inquiry, $id_user).'</span></a>
                            </center>';
                    }else if($mjl->status == 3 || $mjl->status == 4){
                        return '
                            <center>
    ...
    {
        $count = DB::table('csc_chatting_inquiry')->where('id_inquiry', $id)->where('type', 'perwakilan')->where('receive', $receiver)->where('status', 0)->count();
        return $count;
    }
    ...
    public function chatting($id)
    {
        if(Auth::user()){
            ...
            return view('inquiry.perwakilan.chatting', compact('pageTitle','inquiry', 'messages', 'id_user', 'cekfile','data'));
        }else{
            return redirect('/home');
        }
    }
